    ...
    assignEvents() {
        ...
        this.Gaps.forEach(objHTML => {
            if (!!objHTML.dataset.numberOfImage) {
                objHTML.addEventListener('click', function (event) {
                    ...
                    objSelectedT = document.querySelector(`#ThingsTofindList > li[data-number-of-image="${objHTML.dataset.numberOfImage}"]`);
                    if (!!objSelectedT) {
                        if (objHTML.classList.contains('FoundElement')) {
                            objSelectedT.style.textDecoration = 'line-through';
                        }
                        else
                        {
                            objSelectedT.style.textDecoration = 'initial';
                        }
                    }
                }, false);
            }
        ...
        if (!!objfinishedButton) {
            objfinishedButton.addEventListener('click', function (event) {
                ...
                arrThingsTofindList = [].slice.call(document.querySelectorAll(`#ThingsTofindList > li[data-number-of-image]`),0),
                arrIdToFind = arrThingsTofindList.map(TFL=>TFL.dataset.numberOfImage);

                if (!!arrSelectedThings && arrSelectedThings.length == arrIdToFind.length) {
                    ...
                    InitializeGame : function() {
                        ...
                        new AJAXKBB().callController('/Games/getGame', objData, (dataR) => {
                            ...
                            if (objData.IsSuccessful) {
                                ...
                                if (objData.Body.collection.length == 1) {
